...
class Media extends CI_Controller {
	...
	public function index()
	{
		...
		$data['medias'] = $this->media_model->get_all($config['per_page'], $this->uri->segment(4));
	...
	public function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST' )
		{
			...
			// $this->form_validation->set_rules('media_type','Media','required');
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				if ($hasFileUploaded) 
				{
					...
					$this->media_model->create($options);
	...
	public function edit($media_id)
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') 
		{
			...
			$affected = $this->media_model->update($media_id, $options);
			if ($affected)
			{
				if ($hasFileUploaded)
		...
		$data['media'] = $this->media_model->get_by($media_id);
	...
	{
		$this->load->model('media_model');
		$row = $this->media_model->get_by($media_id);
		$newStatus = ($row->status == 'DEACTIVE') ? 'ACTIVE' : 'DEACTIVE' ;
		$options = ['status' => $newStatus];
		$this->media_model->update($media_id,$options);
		redirect('/admin/media','refresh');
	}
	public function delete($media_id)
	{
		...
		$affected = $this->media_model->remove($media_id);
		if ($affected) {
